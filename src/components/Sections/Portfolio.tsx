...
import {usePortfolioItems, SectionId} from '../../data/data';
  ...
  return (
    <Section className="bg-neutral-800" sectionId={SectionId.Portfolio}>
      <div className="flex flex-col gap-y-8">
        <h2 className="self-center text-xl font-bold text-white">{t("porfolio_title")}</h2>
        <div className=" w-full columns-2 md:columns-3 lg:columns-4">
          {usePortfolioItems().map((item, index) => {
...
const ItemOverlay: FC<{item: PortfolioItem}> = memo(({item: {url, title, client}}) => {
    ...
    <a
      ...
      <div className="relative h-full w-full p-4">
        <div className="flex h-full w-full flex-col gap-y-2 overflow-y-auto overscroll-contain">
          ...
          <p className="text-xs text-white opacity-100 sm:text-sm">{client}</p>
        </div>
